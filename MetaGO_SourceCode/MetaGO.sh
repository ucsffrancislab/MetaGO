...
if [ "$InputData" != 'RAW' -a "$InputData" != 'MATRIX' ]
then
echo " ERRO!! Please choose a correct inputdata, you can choose 'RAW' or 'MATRIX' "
exit
fi

if [ -n "$FileList" -a "$InputData" = 'MATRIX' ]||[ -n "$KMER" -a "$InputData" = 'MATRIX' ]||[ -n "$MIN" -a "$InputData" = 'MATRIX' ]||[ -n "$saveUnion" -a "$InputData" = 'MATRIX' ]||[ -n "$Clean" -a "$InputData" = 'MATRIX' ]
then
...
if [ -n "$K2test" -a "$filterFuction" = 'ASS' ]||[ -n "$ASS" -a "$filterFuction" = 'chi2-test' ]
then
...
if [ "$InputData" = 'RAW' ]
then
    if [ -z $FileList ]||[ -z $N1 ]||[ -z $N2 ]||[ -z $KMER ]
    then
...
if [ "$filterFuction" != 'ASS' -a  "$filterFuction" != 'chi2-test' ]
then
echo " ERRO!! Please choose a correct funtion for filtering, you can shoose 'chi2-test' or 'ASS' "
exit
fi

if [ "$Piece" != 16 -a "$Piece" != 8 -a "$Piece" != 4 -a "$Piece" != 1 ]
then
...
then
    ...
    filList=`cat ../Group1FileList.txt`
    for iterm in $filList
    do
    ...
    filList=`cat ../Group2FileList.txt`
    for iterm in $filList
    do
